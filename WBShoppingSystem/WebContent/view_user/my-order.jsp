...
<div class="header container">
	...
	<div class="site-nav">
		<div class="floor_nav">
			...
			<div class="buy_cart">
				<a class="gocart" href="cart.jsp">购物车<b class="yellow">312310</b>件</a>
...
<div class="content container">
	<div class="breadcrumbs">
		<a href="index.jsp">首页</a><span>></span><a href="#">我的麦路客</a><span>></span>我的订单
	</div><!--breadcrumbs end-->
	<div class="f_l personal_l">
		...
		<div class="personal_l_m" id="personal_list">
			<dl>
				...
				<dd>
					<a href="my-order.jsp" class="current">我的订单</a>
				...
				<div class="coll_m">
					...
					<table width="100%">
						...
						<tbody><!-- 
							...
							<tr>
								<td><a href="my-orderdetail.jsp" class="blue">2010091947156</a></td>
								<td><span>2010-09-19 13:43:26</span></td>
								<td>支付宝</td>
								<td><span class="red">￥25.25元</span></td>
								<td>未确认,未付款,未发货</td>
								<td><a href="my-orderdetail.jsp" class="red">[查看]</a> | <a href="#" class="yellow">[支付]</a> | <a href="#" class="blue">[取消]</a></td>
							</tr>
							<tr bgcolor="#f7f7f7">
...
<script>jQuery(function() {
	$.post("../UserController?method=queryOrderByUserId&user_id="+"<%=u.getUser_id() %>", function(data) {
		var arr = JSON.parse(data);
		//alert(arr[0].order_time.Format("yyyy-MM-dd"));
		if(arr==null){
			var info="<tr><td colspan='6'>暂时没有订单信息</td></tr>";
			$("tbody").append(info);
		}else{
			for(var i = 0; i < arr.length; i++) {
				var info1="<tr>"+
					"<td><a class='blue' id='detail' href ='my-orderdetail.jsp?order_id="+arr[i].order_id+"'>"+arr[i].order_id+"</a></td>"+
					"<td><span>"+arr[i].order_time+"</span></td>"+
					"<td>"+arr[i].order_pay+"</td>"+
					"<td><span class='red'>￥"+parseInt(arr[i].order_goods_amount)*parseFloat(arr[i].goods_price)+"</span></td>"+
					"<td>"+arr[i].order_state+"</td>"+
					"<td><a class='red'>[查看]</a> | <a href='#' class='yellow'>[支付]</a> | <a href='#' class='blue' id='del'>[取消]</a></td>"+
				"</tr>	";
				$("tbody").append(info1);
			}
		}
	...
	/*$("tbody").delegate('a','click',function(){
			if($(this).text()=='[查看]'||$(this).attr("class")=="blue"){
			var orderId=$("#detail").text();
			alert(orderId);
			$.post("../UserController?method=queryOrderInfoByOderyId&order_id="+orderId){
				url:
				success:function(req){
			        //请求成功时处理
			    }
			};
		}else if($(this).children('a').attr('id')=='del'){
